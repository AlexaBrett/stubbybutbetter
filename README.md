...
### Tracing UI (Match Debugger)

Enable tracing and open the UI to inspect recent request matches.

1. Start stubby with the UI flag enabled:

```
npm start -- -u -d test/data/e2e.yaml -q
```

2. Build UI assets (one-time or after edits):
...
3. Open the UI:
- http://localhost:8889/ui
- Or go to http://localhost:8889/status and click "Open Match Traces UI"

Endpoints exposed by the admin portal:
- GET /admin/match-traces?limit=10
- GET /admin/match-traces/:id
- POST /admin/match-traces/clear

Notes:
- The UI polls every 2 seconds. Use the filters (Method, Matched) and the Clear button as needed.
- In the Trace detail view, each candidate shows its YAML file and lines, and a link to open the admin stub page (/:id).
- If the request body exceeded the configured max bytes, a "(request body truncated)" indicator is shown.
