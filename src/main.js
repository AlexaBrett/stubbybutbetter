...
const { setTraceConfig } = require('./lib/tracing-state');
...
class Stubby {
  ...
  async start (o) {
    ...
    // Configure tracing based solely on the UI flag; defaults are hardcoded
    setTraceConfig({
      enabled: !!options.ui,
      bufferSize: 100,
      bodyMaxBytes: 65536
    ...
    if (options.ui) {
      ...
      this.adminPortal.on('listening', function () {
        out.status('Admin portal running at http://' + options.location + ':' + options.admin + '/stubbybutbetter/status');
        out.status('Open Match Traces UI at http://' + options.location + ':' + options.admin + '/stubbybutbetter/ui');
      });
