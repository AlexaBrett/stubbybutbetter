...
class Endpoints {
  ...
  find (data, traceContext) {
    ...
    for (id in this.db) {
      ...
      // Prepare candidate trace container if tracing
      let candidate = null;
      if (traceContext) {
        candidate = {
          id: endpoint.id,
          sourceOrder: endpoint.sourceOrder,
      ...
      captures = endpoint.matches(data, candidate);

      if (traceContext && candidate) {
        const firstFail = candidate.checks.find((c) => c.matched === false);
        candidate.summary = {
          allPassed: !!captures,
          firstFail: firstFail ? { field: firstFail.field, reason: firstFail.reason } : undefined
        };
        traceContext.candidates.push(candidate);
      }
